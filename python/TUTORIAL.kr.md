...
class Flow_Hello(uniflow.Uniflow):
    def __init__(self, rt):
        ...
        self.task = self.Task_Hello()      # 모듈의 단일 task
        self.AddTask(self.task)            # flow() 역참조 연결; 아무것도 시작 안 함
...
rt = uniflow.Runtime()
h = Flow_Hello(rt)
h.task.StartFlow()             # task 런칭; 첫 step 은 다음 라운드에 실행
...
- `AddTask(self.task)` 가 task 를 바인딩해서 그 step 들이 `self.flow()` 로 모듈에 닿게 함.
- `h.task.StartFlow()` 가 task 를 런칭; `Entry()` 가 다음 라운드에 실행. 반환값은 `StartResult.Ok`, 이미 task 가 돌고 있으면 `StartResult.Busy`.
...
def on_message(msg):
    handler.current = msg
    handler.task.StartFlow()        # 런칭되면 Ok, 이미 돌고 있으면 Busy
...
생명주기 제어:
- `module.IsIdle` - 비었나? 오케스트레이터가 다른 모듈의 다음 task 를 런칭하기 전에 확인. (`CurrentTaskName()` / `CurrentStepName()` / `CurrentStepOrdinal()` / `CurrentStepDescription()` 으로 어떤 task 가 도는지와 흐름의 위치 조회; idle 이면 `""` / `-1`. `task.Name()` 은 task 핸들에서 같은 클래스 이름.)
