...
UF_Weather::StepResult UF_Weather::OnWeather_Begin()
{
    Describe("submitting GET to ", kKmaUrl);
    std::cout << "[weather] fetching " << kKmaUrl << "\n";
    UF_ASYNC(FetchHtml, std::string(kKmaUrl));
    return UF_NEXT(OnWeather_AfterFetch);
}

UF_Weather::StepResult UF_Weather::OnWeather_AfterFetch()
{
    ...
    html_ = r.value();
    std::cout << "[weather] got " << html_.size() << " bytes of XML\n";
    Describe("got ", html_.size(), " bytes");
    ...
    if (!key || !*key)
    {
        ...
        std::cout << html_.substr(0, 500) << "\n";
        return Done();
    }

    std::string prompt = BuildPrompt(Truncate(html_, kBodyCap));
    std::cout << "[weather] submitting POST to Gemini (" << kModel << ", "
              << "prompt=" << prompt.size() << " bytes)\n";
    Describe("submitting Gemini call");
    UF_ASYNC(CallGemini, std::string(key), std::string(kModel),
             std::move(prompt), kMaxToks);
    return UF_NEXT(OnWeather_AfterLlm);
}

UF_Weather::StepResult UF_Weather::OnWeather_AfterLlm()
{
    auto r = AsyncResult<std::string>();
    if (r.failed())
    {
...
std::string UF_Weather::FetchHtml(std::string url)
{
    return HttpClient::Get(url);
}

std::string UF_Weather::CallGemini(std::string api_key, std::string model,
                                   std::string prompt, int max_output_tokens)
{
    return HttpClient::GeminiGenerate(api_key, model, prompt, max_output_tokens);
}

std::string UF_Weather::BuildPrompt(const std::string& body)
{
...
std::string UF_Weather::Truncate(const std::string& s, std::size_t n)
{
