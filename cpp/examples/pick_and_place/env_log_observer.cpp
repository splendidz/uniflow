...
void EnvLogObserver::OnAsyncAbandoned(std::string_view obj, std::string_view job,
                                      double pending_ms)
{
    std::ostringstream os;
    os << Pad("", kColStep) << " "
       << "[ASYNC ABANDONED]  " << job
       << "  pending=" << FmtMs(pending_ms);
    Emit(obj, os.str());
}

void EnvLogObserver::OnAsyncHighWater(std::string_view obj, std::string_view job,
                                      std::size_t inflight)
{
    std::ostringstream os;
    os << Pad("", kColStep) << " "
       << "[ASYNC HIGHWATER]  " << job
       << " rejected, inflight=" << inflight;
    Emit(obj, os.str());
}
