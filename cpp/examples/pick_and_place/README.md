...
- **The orchestrator pattern** - `Flow_Orchestrator` (a single perpetual `Schedule` task) coordinates the whole line: raw-part spawning (a fresh part is staged the instant zone A is empty) and launching each module's *next task* when it is idle (`task.StartFlow()`, state-driven on `picker.Carrying()` / `stage.state()`). The pickers and the Stage never decide *which task* runs next on their own.
...
All are held as members of `App` with two-phase init: phase 1 constructs every module (ctor bodies do not touch other modules), and phase 2 `Start()` launches the perpetual tasks (`viz.task_snapshot_.StartFlow()`, `orch.task_schedule_.StartFlow()`). Each flow wires its tasks with `AddTask(task_...)` in its constructor (the entry step is named by each task's `Entry()` override); the orchestrator then launches the work tasks via `task.StartFlow()` as each module goes idle.
...
if (p.IsIdle())
{
    if (p.Carrying())                   p.task_place_.StartFlow();   // deliver
    else if (GlobalEnv::ZoneAHasPart()) p.task_pick_.StartFlow();    // fetch
}
```
