...
<p align="center">
  <sub>Left: dozens of cars driving by the signals in <a href="cpp/examples/city_traffic/">city_traffic</a> &nbsp;|&nbsp; Right: two pickers running a line with no zone collision in <a href="cpp/examples/cnc_pickers/">cnc_pickers</a></sub><br>
...
| `UF_STAY_UNTIL` | Poll the current step, but route to a cleanup step once a deadline passes (a step-level catch) |
...
[Open page](cpp/examples/city_traffic/README.md) | [Folder](cpp/examples/city_traffic/)
...
[Open page](cpp/examples/cnc_pickers/README.md) | [Folder](cpp/examples/cnc_pickers/)

### The rest

| Example | One line | Difficulty | UI |
|---|---|---|---|
| [shared_ostream](cpp/examples/shared_ostream/README.md) | Two modules share one ostringstream with no locks (minimal example) | ★☆☆☆☆ | Console |
...
**Visual Studio**: `cpp/examples/*/<name>.vcxproj` works out of the box; `AdditionalIncludeDirectories=..\..\` is all it needs.

### Run it in your browser

To try the quick tutorials with no local setup, open the repo in GitHub Codespaces (badge below) and in the terminal:

[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/splendidz/uniflow)

```bash
g++ -std=c++17 -O2 -pthread -I cpp cpp/examples/quickstart/tut1_roundrobin.cpp -o tut1 && ./tut1
```

The compilable single-file versions of all three quick tutorials live in [cpp/examples/quickstart/](cpp/examples/quickstart/), and CI builds/runs them on Windows/Linux/macOS on every commit.
