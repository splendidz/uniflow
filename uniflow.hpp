...
//
// It governs ONLY logical waits. Async / IO deadlines (UF_ASYNC_TIMEOUT) and
// the pump's own sleeps stay on real wall-clock - a network call does not
...
{
public:
    VirtualClock() { base_real_ = Clock::now(); base_virtual_ = base_real_; }
    ...
    double Scale() const { std::lock_guard<std::mutex> lk(mu_); return scale_; }
    ...
    {
        std::lock_guard<std::mutex> lk(mu_);
        if (frozen_) { base_real_ = Clock::now(); frozen_ = false; }
    }
    bool Frozen() const { std::lock_guard<std::mutex> lk(mu_); return frozen_; }
...
private:
    TimePoint now_locked() const
    {
        ...
        std::chrono::duration<double, std::nano> real_elapsed =
            Clock::now() - base_real_;
        return base_virtual_
               + std::chrono::duration_cast<Clock::duration>(real_elapsed * scale_);
    }
...
// can answer 'who started this flow?'. The directory part of the path is
// stripped at capture time - log lines stay readable without leaking
...
// path has no separator, return the whole thing unchanged. constexpr so
// UF_START_FLOW can do the stripping at compile time on the literal that
// __FILE__ expands to.
...
public:
    ...
    // source location of the UF_START_FLOW caller (file/line/function blank
    // when StartFlow was called without the macro).
    ...
    virtual void OnAsyncCompleted([[maybe_unused]] std::string_view obj,
                                  [[maybe_unused]] std::string_view job,
                                  [[maybe_unused]] double wait_ms,
                                  [[maybe_unused]] bool had_error,
    ...
    virtual void OnFlowEnded([[maybe_unused]] std::string_view obj,
                             ...
                             [[maybe_unused]] double total_step_ms,
...
public:
                       ...
                       const TickStats& step_ticks) override
    {
        ...
        std::cout << "[" << pad(obj, kColObj) << "] "
                  << pad(transition, kColStep) << " "
                  << pad(description, kColDesc) << " "
                  << "#" << pad2(step_ordinal)
                  << " elapsed=" << fmt_ms(elapsed_ms)
                  << "  tick x" << step_ticks.count
                  << " avg=" << fmt_ms(step_ticks.avg_ms)
                  << " min=" << fmt_ms(step_ticks.min_ms)
                  << " max=" << fmt_ms(step_ticks.max_ms)
                  << "\n";
    }
    ...
    void OnFlowEnded(std::string_view obj, StepAction terminal_action,
                     ...
                     const FlowTickSummary& flow_ticks,
                     ...
                     FlowOrigin origin) override
    {
        ...
        std::cout << "[" << pad(obj, kColObj) << "] FLOW "
                  ...
                  << " avg=" << fmt_ms(flow_ticks.all.avg_ms)
...
public:
    ...
    // StartFlow and Post already call this; call it yourself after mutating
...
private:
    ...
    // round.
    enum class RoundOutcome { Idle, Staying, Advanced };
    ...
    void pump_loop()
    {
        ...
        for (;;)
        {
            ...
            switch (sleepLevel)
            {
            case RoundOutcome::Advanced: nap_ms = config_.step_interval_sleep_ms; break;
            case RoundOutcome::Staying:  nap_ms = config_.stay_sleep_ms;          break;
            case RoundOutcome::Idle:     nap_ms = config_.idle_sleep_ms;          break;
            }
            // Interruptible rest. Wake() (from StartFlow / Post / a user
            ...
            if (nap_ms > Duration::zero())
            {
                ...
                wake_cv_.wait_for(lk, nap_ms, [this] {
...
public:
    ...
    void Restart() { armed_at_ = clk_->Now(); cond_since_ = TimePoint{}; }
...
public:
    ...
    struct StepResult
    {
        ...
        // Next: the step to advance to. StayUntil: the timeout target to
        // route to when the step deadline passes. Unused otherwise.
        std::function<StepResult()> next_fn;
        const char*                 next_name = nullptr; // name of next_fn for the log
        // StayUntil only: deadline measured from step entry. Zero (the
        // default) means "plain Stay, no step timeout".
    ...
    {
        return StartFlowAt(nullptr, 0, nullptr, fn, std::forward<Args>(args)...);
    }
    template <class... Params, class... Args>
                     ...
                     StepResult (Derived::*fn)(Params...), Args&&... args)
    {
        ...
        flow_origin_          = FlowOrigin{origin_file, origin_line, origin_function};
        // Nudge the pump so the entry step runs on the next round instead of
        // after the current nap - the whole point of starting a flow from an
...
protected:
    ...
    //
    // Stay()   - re-run this step on the next pump round. Pump cadence
    //            between rounds is set by Config::stay_sleep_ms (default
    //            20ms). For a longer wait, hold a TimePoint in the
    //            module and return Stay() until enough time has passed.
    ...
    // StayUntil(timeout, fn, args...) - a Stay that carries a step deadline.
    // The CURRENT step keeps polling (same as Stay()); but if 'timeout'
    // elapses since the step was ENTERED (not since this call - repeated
    // Stay ticks do not reset it), the flow transitions to step 'fn'
    // instead. This is the step-level "catch": route to a cleanup / recovery
    // step rather than polling a condition that may never arrive. Use through
    // UF_STAY_UNTIL so the target name shows in logs.
    template <auto Fn, class... Args>
    StepResult StayUntil(Duration timeout, const char* name, Args&&... args)
    {
        StepResult r = Next<Fn>(name, std::forward<Args>(args)...);
        r.action  = StepAction::Stay; // it is a Stay that also holds a deadline
        r.timeout = timeout;
        return r;
    }
...
bool Uniflow<Derived>::ExecuteOnce(IUniflowObserver& obs, RoundProfile* prof)
{
    ...
    if (async_pending_)
    {
        ...
        bool timed_out = pending_timeout_ != Duration::max()
                         && elapsed > pending_timeout_;
        ...
        if (!ready && !timed_out)
        {
            ...
            if (!slow_warned_
                && cfg.slow_async_threshold_ms != Duration::max()
        ...
        if (timed_out && !ready)
        {
            ...
            last_async_exception_ = std::make_exception_ptr(
                AsyncTimeout{pending_job_label_, to_duration(elapsed)});
    ...
    if (cpu_dt < step_tick_min_) step_tick_min_ = cpu_dt;
    if (cpu_dt > step_tick_max_) step_tick_max_ = cpu_dt;
    step_tick_sum_ += cpu_dt;
    if (cpu_dt < flow_tick_min_) flow_tick_min_ = cpu_dt;
    ...
    if (!step_had_error
        && cfg.slow_step_threshold_ms != Duration::max()
    ...
    case StepAction::Fail:
    {
        ...
        const char* next_name =
            r.action == StepAction::Next && r.next_name ? r.next_name : "";
        ...
        step_ticks.min_ms = step_tick_min_ == Clock::duration::max()
                            ? 0.0 : to_ms(step_tick_min_);
        step_ticks.max_ms = to_ms(step_tick_max_);
        step_ticks.avg_ms = ticks_in_step_ > 0
                            ? to_ms(step_tick_sum_) / ticks_in_step_
                            : 0.0;

        obs.OnStepChanged(instance_name_,
                          prev_name, next_name,
        ...
        else
        {
            ...
            fts.all.min_ms = flow_tick_min_ == Clock::duration::max()
                             ? 0.0 : to_ms(flow_tick_min_);
            fts.all.max_ms = to_ms(flow_tick_max_);
            fts.all.avg_ms = tick_count_ > 0
                             ? to_ms(total_cpu_) / tick_count_
                             : 0.0;
            fts.max_step   = flow_tick_max_step_
                             ? std::string_view(flow_tick_max_step_)
                             : std::string_view();

            obs.OnFlowEnded(instance_name_, r.action,
                            step_ordinal_, trace_,
                                   ...
                                   Args&&... args)
{
    ...
    // Captured so the worker can Wake() the pump the instant the job finishes,
    // instead of the gated step waiting out a stay_sleep poll to notice.
    Runtime* rt = runtime_;
    runtime_->executor().Submit(
        [promise, rt, tup = std::move(tup)]() mutable
        {
            try
            {
            ...
            rt->Wake(); // completion (value or exception) is ready - poke the pump
        });
}
...
// Keep polling the CURRENT step, but if 'dur' elapses since the step was
// entered, route to step 'fn' (the timeout / cleanup target). 'dur' is a
// uniflow::Duration (the chrono literals 1ms / 1s work). Optional trailing
// args are forwarded to 'fn'.
#define UF_STAY_UNTIL(dur, fn, ...) \
    this->template StayUntil<&S::fn>((dur), #fn, ##__VA_ARGS__)
