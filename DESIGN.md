...
> **문서 상태** — 이 설계는 [include/uniflow.hpp](include/uniflow.hpp)에 구현 완료됐고, [examples/](examples/)의 두 예제로 검증됐다. 구현하며 추가로 확정된 사항은 §9에 정리. 구버전 튜토리얼·데모는 [temp/](temp/)에 보관(참조하지 않음).
...
| 파일 | 역할 |
|---|---|
| [include/uniflow.hpp](include/uniflow.hpp) | 프레임워크 전체 (header-only, C++17). §2/§5 개정 컨셉 반영 완료 |
| [include/BS_thread_pool.hpp](include/BS_thread_pool.hpp) | 벤더링한 BS::thread_pool v4.1.0. 선택 — `UNIFLOW_USE_BS_THREAD_POOL` 정의 시 |
| [examples/message_dispatch/](examples/message_dispatch/) | 예제 1 — 학생/스케줄러 메시지 분배 (콘솔). step 체인 + async + 다중 클라이언트 |
| [examples/cnc_pickers/](examples/cnc_pickers/) | 예제 2 — CNC 피커 (Win32 시각화 + 콘솔 폴백). 1클래스 2인스턴스 + 충돌 회피 |
...
이 문서를 읽고 → §5 결정 10개 + §9 "구현하며 확정된 것" → [include/uniflow.hpp](include/uniflow.hpp) 헤더 주석 → [examples/](examples/)의 두 예제 순으로 보면 컨텍스트가 복원된다.
