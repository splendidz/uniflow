...
- **flow / task라는 강제된 골격.** 개발자가 OOP 관점으로 - "이 모듈은 한 번에 하나의 상태", "이 작업은 이 단계들로 구성" - 사고하도록 유도한다. 기능과 코드가 늘어도 전부 같은 형태라 리뷰가 일관된다. 개발자의 역량이나 취향과 무관하게 같은 구조로 짜이므로, 프로젝트가 시간이 지나며 스파게티가 되는 것을 구조적으로 예방한다.
...
**Flow** 는 "동시에 두 가지 상태가 될 수 없는 단위"를 모델링한다. 자동차는 앞으로 가면서 동시에 뒤로 갈 수 없다 - 자동차 한 대가 하나의 `Flow` 클래스다. 이벤트 프로세서는 두 이벤트를 동시에 처리할 수 없다 - 그 프로세서가 하나의 `Flow`다. 물리적 장치, 채널, 논리적 배타 자원 - 한 번에 하나의 작업만 돌아갈 수 있다면 모두 `Flow`로 표현할 수 있다.

**Task** 는 그 `Flow` 안에서 수행하는 구체적인 작업 단위이다. 자동차라면 "전진", "후진"이 Task고, 이벤트 프로세서라면 "이벤트 타입 A 처리"가 Task다. 하나의 `Flow`에 여러 Task를 선언해두면, 어떤 Task가 실행 중인지에 따라 모듈의 현재 역할이 결정된다. Task들은 서로 교대하지만 같은 시간에 두 개가 동시에 돌지는 않는다 - 이것이 `Flow` 배타성의 핵심이다.

선언부와 본문을 분리해서 보면 구조가 한눈에 들어온다. 선언(`struct`)은 흐름의 골격이고, 본문은 그 아래에 둔다(실제 프로젝트에선 `.cpp`).
...
StepResult Step4_WaitAck()
{
    ...
    return StayUntil(3000ms, UF_FN(Step5_Timeout));     // 3초 안에 ack 없으면 타임아웃 스텝으로
}
...
위 플래그 버전에서 폭발하던 것 - `connected_`/`connecting_`/`cmd_sent_`/`waiting_ack_`/`draining_`/`fault_`/`estop_` 가 서로의 리셋을 암묵적으로 책임지고, e-stop 과 fault 가 어느 분기에서도 끼어들 수 있어 "리셋 목록을 빠뜨리면 버그"가 되던 구조 - 가 사라진다. 중괄호 depth가 고정되고, 각 상태는 이름 있는 스텝 하나가 된다. e-stop/fault 같은 횡단 경로조차 플래그가 아니라 명시적 전이(`StayUntil`/`Next`/`Fail`)로 표현되므로, 단계를 추가할 때 함수 하나를 더하고 연결만 바꾸면 되며 나머지 단계는 손대지 않는다.
...
시뮬레이터를 uniflow로 구현하면 **전체 시뮬레이션의 배속과 정지를 별도 구현 없이 얻는다.** 모든 시간 기반 로직 - 스텝 타임아웃(`StayUntil`), 경과/세틀 타이머(`UFTimer`, `HeldFor`) - 이 `Runtime`이 들고 있는 하나의 논리 시계를 따르기 때문이다. 그 시계 하나를 배속하거나 얼리면 위의 모든 flow가 함께 빨라지거나 멈춘다.

<!-- 다이어그램: 논리 시계 1개가 모든 flow의 StayUntil/UFTimer를 구동 (SetScale/Freeze가 전체에 전파) -->
...
**여러 작업을 동시에** 던지고 한 단계에서 모두 기다릴 수 있다. `AnyAsyncPending()`이 join-all 프리미티브다(데드라인은 `StayUntil`로 직접).
...
폴링하는 조건 자체에 타임아웃이 필요한 경우에도 동일하게 `StayUntil`을 쓴다.

```cpp
StepResult Step1_WaitSensor()
{
    if (flow().sensor_.IsReady()) return Next(UF_FN(Step2_Process));
    return StayUntil(3000ms, UF_FN(Step3_SensorTimeout));  // 3초 초과 시 타임아웃 단계로
}
```
...
세 Task를 가지고, 명령 ack는 `SubmitAsync` 후 `AsyncResult` 폴링 + `StayUntil` 타임아웃으로
받는다.
