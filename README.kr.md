...
<p align="center">
  <sub>왼쪽: 차량 수십 대가 신호를 보며 도는 <a href="cpp/examples/city_traffic/">city_traffic</a> &nbsp;|&nbsp; 오른쪽: 픽커 2대가 zone 충돌 없이 라인을 운전하는 <a href="cpp/examples/cnc_pickers/">cnc_pickers</a></sub><br>
...
`uniflow`는 검증된 **reactor 패턴**과 **event loop + worker pool 모델**(Node.js / libuv 계보)을 C++ 타입 시스템 위에 *구조적으로 강제*한 단일 헤더 프레임워크다. 새 패러다임이 아니라, 오래 검증된 동시성 모델을 step 함수라는 형태로 정리해 손에 쥐어 준다.
...
| `UF_STAY_UNTIL` | 현재 step을 폴링하되 진입 후 기한 초과 시 정리 step으로 자동 전이 (step 단위 catch) |
...
[페이지 보기](cpp/examples/city_traffic/README.kr.md) | [폴더](cpp/examples/city_traffic/)
...
[페이지 보기](cpp/examples/cnc_pickers/README.kr.md) | [폴더](cpp/examples/cnc_pickers/)

### 나머지 예제

| 예제 | 한 줄 | 난이도 | UI |
|---|---|---|---|
| [shared_ostream](cpp/examples/shared_ostream/README.kr.md) | 모듈 둘이 한 ostringstream을 락 없이 공유 (최소 예제) | ★☆☆☆☆ | 콘솔 |
...
**Visual Studio**: `cpp/examples/*/<name>.vcxproj` 가 즉시 동작. `AdditionalIncludeDirectories=..\..\` 만 설정되어 있으면 충분하다.
...
세 퀵 튜토리얼의 컴파일 가능한 단일 파일은 [cpp/examples/quickstart/](cpp/examples/quickstart/)에 있고, CI가 매 커밋마다 Windows/Linux/macOS에서 이들을 빌드/실행한다.
