...
UF_Vehicle::UF_Vehicle(uniflow::Runtime& rt, int id, int from, int to,
                       double dist0, uint8_t r, uint8_t g, uint8_t b)
    : uniflow::Uniflow<UF_Vehicle>(rt),
      id_(id), from_(from), to_(to), dist_(dist0),
      r_(r), g_(g), b_(b),
      last_(uniflow::Clock::now()),
      rng_(static_cast<unsigned>(id * 2654435761u + 12345u))
{
    UpdateHeading();
}

double UF_Vehicle::Dt()
{
...
void UF_Vehicle::ApproachSpeed(double target, double dt)
{
...
double UF_Vehicle::GapAhead(int next) const
{
...
double UF_Vehicle::NextEdgeRoom(int next) const
{
    const std::vector<city::VehicleState>& vs = city::Vehicles();
    double                                 best = 1e9;
    for (int i = 0; i < static_cast<int>(vs.size()); ++i)
...
int UF_Vehicle::TurnDir(int n) const
{
...
UF_Vehicle::StepResult UF_Vehicle::OnSeg_Cruise()
{
    if (sim::Stop())
        return Done();
    ...
    if (dist_ >= stopline - 1e-9)
    {
        if (mayGo && !exitBlocked)
            return UF_NEXT(OnSeg_Cross);
        speed_ = 0.0;
        return UF_NEXT(OnSeg_Wait);
    }
...
UF_Vehicle::StepResult UF_Vehicle::OnSeg_Wait()
{
    if (sim::Stop())
        return Done();
...
UF_Vehicle::StepResult UF_Vehicle::OnSeg_Cross()
{
    if (sim::Stop())
        return Done();
...
UF_Vehicle::StepResult UF_Vehicle::OnSeg_Turn()
{
    if (sim::Stop())
        return Done();
