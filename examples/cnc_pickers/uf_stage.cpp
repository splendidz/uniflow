...
UF_Stage::StepResult UF_Stage::OnProcess_WaitHwReady(uniflow::UFTimer& t)
{
    ...
    // handshake), not on the first transient high. 't' is carried by-ref
    // through UF_NEXT so its sustain accumulator survives the Stay re-entries.
    if (t.HeldFor(HwSimulator::IsReady(), 50ms))
    {
        Describe("processing");
    ...
    Describe("wait hw ready");
    return UF_STAY_UNTIL(3000ms, OnProcess_WaitHwReadyTimeout);
}

UF_Stage::StepResult UF_Stage::OnProcess_WaitHwReadyTimeout()
{
